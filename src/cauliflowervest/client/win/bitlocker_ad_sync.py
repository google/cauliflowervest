...
LDAP, and escrows to CauliflowerVest using HTTP Over RPC.
...
# TODO(user): polish credential storage/retrieval for open source.
# TODO(user): add setuptools build, which installs ldap and gflags.
...
class InvalidRecoveryGuid(Error):
  """An invalid RecoveryGUID value was encountered."""
...
class PreviouslyEscrowed(Error):
  """The recovery object was previously escrowed."""
...
class BitLockerAdSync(object):
  ...
  def SyncDn(self, base_dn=None, ldap_filter=None, skip_usn_changed=None):
    ...
    for result in self._QueryLdap(base_dn, ldap_filter):
      ...
      except PreviouslyEscrowed as e:
        logging.debug('Skipping previous escrowed GUID: %r', str(e))
      except InvalidDistinguishedName as e:
        logging.debug('Skipping object with invalid DN: %r', str(e))
        continue
      except InvalidRecoveryGuid as e:
        logging.debug(
            'Skipping object with invalid recovery GUID %r', str(e))
        continue
